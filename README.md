...
- 物品：按類型（武器/防具/飾品/丹藥/飲食/秘籍/珍寶/材料/馬匹）、品質、子類篩選添加，自動填充基礎屬性
  - **價格自動同步**：修改物品等級（品級）或品質（稀有度）時，價值會根據遊戲公式自動重新計算
...
#### 門派
- 門派資訊：門派名稱、等級、行事風格、掌門、總人口、總薪俸
- 資源管理：六項資源（銀兩/糧食/木料/礦石/藥材/威望）的儲量、上限、月變化
- 佔領區域：顯示門派所有佔領地區
- 外交系統：
  - 同盟管理：勾選結盟/解盟，支援多門派同盟，雙向同步，批量全部結盟/全部解盟
  - 附庸關係：設定宗主門派、添加/移除附庸門派，雙向同步（宗主↔附庸），支援批量解除
  - 外交好感設定（forceFavorSetting）：門派 AI 外交好感目標設定，可添加/移除/清除
  - 停戰協議（ForceStopWarTime）：設定停戰天數，雙向同步，支援批量全部停戰/全部解除
  - 外交好感度：對所有門派的好感值（0–100），顯示關係等級（友好/親善/中立/冷淡/敵對），支援批量設定（全部友好/中立/敵對）
  - 綜合外交表：一覽各門派好感度、同盟狀態、附庸關係、停戰狀態

#### 外部倉庫
- 獨立標籤頁，無需載入存檔即可使用
- 物品管理：支援跨存檔共享物品
- 新增物品：按類型（武器/防具/飾品/丹藥/飲食/秘籍/珍寶/材料/馬匹）篩選添加
- 物品編輯：的名稱、等級、品質、價值、重量、毒素屬性
  - **價格自動同步**：修改物品等級或品質時，價值自動重新計算
- 強化屬性：裝備的基礎屬性（黑字）和詞條屬性（綠字）編輯
- 藥食效果：丹藥/飲食的回血、回复內力、回复體力、治療內外傷、解毒等效果
- 坐騎屬性：速度、力量、衝刺、抗性
- 批量操作：批量刪除物品
- 導入/導出：載入外部倉庫 `.dat` 檔案、導出保存
